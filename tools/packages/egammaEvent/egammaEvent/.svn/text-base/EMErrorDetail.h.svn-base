...
class EMErrorDetail : public egDetail
{
 public:
  ...
  /** @brief Fill the details of the object. This is the main function for building the detail*/
  void fillEMErrorDetail(const egamma*, const Trk::ParametersBase*, const EMClusterErrorsParametrizations*, 
			 const HepGeom::Point3D<double>&);
  
  /** @brief Fill and get the track covariance matrix from the method where track and cluster information are combined*/
  bool fillEMTrackCombinedErrorMatrix(const CLHEP::HepSymMatrix& combinedMatrix);

  /** @brief E/P combined error matrix, natural size */
  CLHEP::HepSymMatrix getCombinedErrorMatrix() const;
  /** @brief E/P combined error matrix, natural size */
  CLHEP::HepSymMatrix getUncombinedErrorMatrix() const;

  /** @brief E/P combined error matrix, (E,eta,phi,M) representation */
  CLHEP::HepSymMatrix get4x4CombinedErrorMatrix() const;
  /** @brief E/P combined error matrix, (E,eta,phi,M) representation */
  CLHEP::HepSymMatrix get4x4UncombinedErrorMatrix() const;
  ...
  CLHEP::HepSymMatrix getEMTrackCombinedErrorMatrix() const;
  /** @brief Returns the ErrorMatrix of photons--Depricated for public use; */
  CLHEP::HepSymMatrix getEMPhotonErrorMatrix() const;
...
private:
  ...
  double getClusterPhiError(const egamma*, const EMClusterErrorsParametrizations*, bool forcePhoton = false) const;
   ...
   /** @brief Fill the perigree parameter for converted photon */
  bool fillConvertedPhotonMatrixElements(const Trk::ParametersBase*);
  /** @brief Fill the perigree parameter for converted photon  */
  bool fillConvertedPhotonErrorMatrix(const Trk::ErrorMatrix&);

   /** @brief Fill the perigree parameter details */
  bool fillTrackPerigeeMatrixElements(const Trk::ParametersBase*);
  /** @brief Fill the perigree error details */
  bool fillTrackPerigeeErrorMatrix(const Trk::ErrorMatrix&);

  /** @brief Fill the photon parameter details */ 
  bool fillPhotonMatrixElements(const egamma*, const EMClusterErrorsParametrizations*, 
				const HepGeom::Point3D<double>&, bool forcePhoton=false);
  /** @brief Fill the perigree error details */
  ...
  // jacobians
  CLHEP::HepSymMatrix P5Jacobiand0z0PhiThetaE2d0z0PhiEtaE(const double phi) const;
  CLHEP::HepSymMatrix P4JacobiandEThetaPhiM2EEtaPhiM(const double theta) const;
