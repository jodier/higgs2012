////////////////////////////////////////////////////
// create plot for 7 TeV datasets
//
// Kostas 
//
////////////////////////////////////////////////////
...
int createAplot2ppnew(int typein,int bin)
{
  ...
  TH1F* tmp[5];
  double tmpuncert[5]={0.10,0.60,0.5,0.25,0.7};
  if(type==0)
    {
  ...
  **/
  ...
  TString thetex("#scale[0.55]{#int}Ldt = 4.8 fb^{-1}");// 1.37-1.61 fb^{-1}");
  if(type==2)
    thetex="#scale[0.55]{#int}Ldt = 4.9 fb^{-1}";
  //TLatex*tex = new TLatex(0.23,0.57,thetex);
  TLatex*tex = new TLatex(0.26,0.50,thetex);
  tex->SetNDC(kTRUE);
  tex->SetTextFont(42);
  ...
  tex->Draw();
  TString thetex2("#sqrt{s} = 7 TeV");
  TLatex*tex2 = new TLatex(0.28,0.44,thetex2);
  tex2->SetNDC(kTRUE);
  tex2->SetTextFont(42);
  ...
  TLatex *pt = new TLatex(0.27,0.56,name);
  if(type==6)
    pt->SetX(0.24);
  pt->SetNDC(kTRUE);
  pt->SetTextFont(42);
  ...
  if(docomp)
    outname+="_comp";
  c1->Print(outname+"_2011.eps");
  c1->Print(outname+"_2001.png");
  return 0;
}
