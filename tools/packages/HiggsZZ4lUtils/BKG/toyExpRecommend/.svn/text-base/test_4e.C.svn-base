...
int toymc()
{
  ...
  const int Ncat    = 9;   // number of experimental categories for pairs
  const int Ncatm  = 16;  // number of mctruth categories for pairs
  const int nExp = 5000;   // number of pseudoexperiments
  //TString outname("bkg4eSameEff"); 
  TString outname = "bkg4eNoExchange";
  ...
  fstream filestr ("purityPoisson_noExchange.txt", fstream::in);
  char txt00[200];
  ...
  fstream filestr2 ("effi_led.txt", fstream::in);
  //char txt01[100];
  filestr2.getline(txt01,100);
  std::cout<< txt01<<"\n";
  double effi_led[Ncatexp][Ncatmc]  = { 0.};
  double eeffi_led[Ncatexp][Ncatmc] = { 0.};
  for(int icount=0;icount<Ncatexp;icount++)
    {
      char txt1[2];
      double nevents;
      filestr2 >>txt1>> effi_led[icount][0]>>eeffi_led[icount][0] //ebcf
	      >> effi_led[icount][1]>>eeffi_led[icount][1]
	      >> effi_led[icount][2]>>eeffi_led[icount][2]
	      >> effi_led[icount][3]>>eeffi_led[icount][3];
	//std::cerr<<effi[icount][3]<<std::endl;
    }
  filestr2.close();
  ...
  for(int i = 0; i < Ncat; i++){
    for (int j = 0; j < Ncatm; j++){
      ...
      effitabl[i][j] = effi_led[indexExpOther][indexTruthOther]* effi[indexExp][indexTruth];
      //effitabl[i][j] = effi[indexExpOther][indexTruthOther]* effi[indexExp][indexTruth];
    }
  }
  ...
  h->SetStats(kTRUE);
  h->Draw();
  c->Print(Form("%s.png",outname.Data()));
  c->Print(Form("%s.eps",outname.Data()));
  return 0;
}
