...
const TString& Root::TAccept::getCutName( unsigned int cutPosition ) const
{
  ...
  std::map< const TString, std::pair< TString, unsigned int > >::const_iterator it    = m_cutMap.begin();
  std::map< const TString, std::pair< TString, unsigned int > >::const_iterator itEnd = m_cutMap.end();
  for ( ; it != itEnd; ++it )
    {
...
{
  // iterate over the map and find the right position
  std::map< const TString, std::pair< TString, unsigned int > >::iterator it    = m_cutMap.begin();
  std::map< const TString, std::pair< TString, unsigned int > >::iterator itEnd = m_cutMap.end();
  for ( ; it != itEnd; ++it )
    {
      if ( (it->second).second == cutPosition )
        {
