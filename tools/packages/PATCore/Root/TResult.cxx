...
const TString& Root::TResult::getResultName( unsigned int resultPosition ) const
{
  ...
  std::map< const TString, std::pair< TString, unsigned int > >::const_iterator it    = m_resultMap.begin();
  std::map< const TString, std::pair< TString, unsigned int > >::const_iterator itEnd = m_resultMap.end();
  for ( ; it != itEnd; ++it )
    {
...
{
  // iterate over the map and find the right position
  std::map< const TString, std::pair< TString, unsigned int > >::iterator it    = m_resultMap.begin();
  std::map< const TString, std::pair< TString, unsigned int > >::iterator itEnd = m_resultMap.end();
  for ( ; it != itEnd; ++it )
    {
      if ( (it->second).second == resultPosition )
        {
