...
namespace Root {
  ...
  public:
    ...
    inline unsigned int getResultPosition( const TString& resultName ) const 
     {
      std::map< const TString, std::pair< TString, unsigned int > >::const_iterator it = m_resultMap.find(resultName);
  ...
  private:
    ...
    std::map< const TString, std::pair< TString, unsigned int > > m_resultMap;
