{
  ...
  fInputs.cd("ShowerShapes");e0dataHist.Draw("esame");fInputs.cd("Scales");
  ...
  TList* le = (TList*) fInputs.Get("PSRecalibration/ElectronGraphs");
  TList* lu  = (TList*) fInputs.Get("PSRecalibration/UnconvertedGraphs");
  TList* lc = (TList*) fInputs.Get("PSRecalibration/ConvertedGraphs");
  ...
  for(int i=0; i<le->GetSize(); i++) {
    ...
    TH1D* hbins = (TH1D*) fInputs.Get("PSRecalibration/ElectronEtaBins");
