...
TGraph* plotScaleCorrection_ERUpgrade(double Energy /* GeV */, double etamin, double etamax, string year) {

   gROOT->ProcessLine(".L EnergyRescalerUpgrade.cxx");
  
   egRescaler::EnergyRescalerUpgrade  ers;
   ers.Init("../share/EnergyRescalerData.root",year,"es2010");
   ...
   char grafname[99];
   sprintf(grafname,"Data calibration factor, %s",year.c_str());
